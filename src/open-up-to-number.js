...
export default function openUpToNumber(event) {
	...
	for (let i = 0; i < fild1.children.length; i++) {
		for (let j = 0; j < fild1.children[i].children.length; j++) {
			if (event == fild1.children[i].children[j]) {
				if (fild1.children[i - 1] == undefined || event.textContent !== ' ' || fild1.children[i - 1].children[j].className !== ('button' + '-' + spunLevelThis.textContent.toLowerCase())) {
					return
				}
